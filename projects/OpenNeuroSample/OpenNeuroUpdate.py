...
class OpenNeuroUpdate(object):
    ...
    def get_dataset_description(self):
        description_path = self.path + "/dataset_description.json"
        try:
    ...
    def get_bids_required_info(self):
        ...
        if self.image_info is not None:
            ...
            print(pathinfo)
            return pathinfo[0], pathinfo[-3], pathinfo[-1]
    ...
    def dataset_Bidsinc(self, sliceIndex: int = 0):
        ...
        datasetMetadata = self.get_dataset_description()
        metadata = self.get_metadata()
        if metadata == {}:
            dataset_metadata_path = self.path + "/{}_{}.json".format(task, suffix)
            print("dataset metadata path", dataset_metadata_path)
            try:
        ...
        imageMetadata = {'subject': subject, 'session': self.conf['sessionId'], 'task': task,
                         'suffix': suffix, 'datatype': self.conf['dataType']}
        imageMetadata.update(metadata)
...
def edit_conf(conf, save_path = None):
    ...
    conf['dataType'] = "anat" # image type: anat or func
...
"""
# testing
dataset = ('./ds000102_sub04','./ds000102_sub04/open_neuro.toml', "sub")
update = OpenNeuroUpdate(dataset)
update.dataset_Bidsinc()
"""
